...
if ($count > 0 && !$open) {
	...
	// update it
	$db->update("space_state", array("open"=>1, "trigger_message"=>$trigger_message, 'last_update'=>time()), "open = ?", 0);
...
// close it
	...
	// update it
	$db->update("space_state", array("open"=>0, "trigger_message"=>$trigger_message, 'last_update'=>time()), "open = ?", 1);
